...
export default function ProductsPage({ urlSlug }: Readonly<{ urlSlug?: string }>) {
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'start', padding: '0px', gap: '20px', width: '100%' }}>
                ...
                <ProductPageTextFilter categories={categories} selectedCategory={activeCategoryId} isBio={isBio} isLoading={spinLoading} orderChangeAction={(order) => { setSorting(order); }} bioChangeAction={(bio) => { setIsBio(bio); }} onCategoryChangeAction={handleCategoryChange} filterChangeAction={(text) => {
                    if (text.length > 0) {
                        const filteredProducts = products.filter(p => p.name.toLowerCase().includes(text.toLowerCase()));
                    ...
                    {loading !== false ? (
                        ...
                        <>
                            {productList.length > 0 ? productList.map(product => (
                                <Grid size={{ xs: 12, sm: 4, md: 3, lg: 2.4 }} key={product.id} sx={{ transition: 'all 0.3s ease' }}>
        ...
        <Box sx={{
            ...
            <Select
                ...
                sx={{
                    ...
                    width: { xs: '100%', sm: 'initial', md: 'initial', lg: 'initial', xl: 'initial' },
                    minWidth: '200px',
                    backgroundColor: '#fff',
                    borderRadius: '4px',
                    height: '38px',
            ...
            <Box sx={{ display: 'flex', gap: '16px', flexDirection: 'row', alignItems: 'center', width: '100%' }}>
                <IconButton
                    sx={{
                    ...
                    <F2FIcons name="BioBadge" width={32} height={32} style={{ alignContent: 'center' }} />
                </IconButton>
                ...
                <TextField
                    ...
                    slotProps={{
                        input: {
                            startAdornment: (
                                <InputAdornment position="start" sx={isLoading ? { animation: 'rotation 2s infinite linear', animationDirection: 'reverse' } : {}}>
