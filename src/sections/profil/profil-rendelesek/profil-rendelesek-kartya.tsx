...
import { Paper, Stack, Typography, Chip, Avatar, Box, Grid } from "@mui/material";
...
export default function ProfilRendelesekKartya({ order }: Readonly<{ order: IOrder }>) {
    ...
    return (
        <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 }, borderRadius: '4px', borderColor: '#e0e0e0', width: '100%' }}>
            <Grid container spacing={2} alignItems="center">
                <Grid size={{xs:12, md:7}}>
                    <Stack spacing={1}>
                        <Typography variant="body1" sx={{ fontSize:"18px", lineHeight:"28px", fontWeight:700 }}>
                            Rendelés dátuma: {order.orderDate}
                        </Typography>
                        <Typography variant="body2" color="text.secondary" sx={{ fontSize:"16px", lineHeight:"24px", fontWeight:500 }}>
                            ...
                            {displayProducts.map((product, index) => {
                                ...
                                return (
                                    <Grid size={{xs:4}} key={product.id}>
                                        <Box sx={{ position: 'relative', '&:hover': { transform: 'scale(1.05)', transition: 'transform 0.2s ease-in-out' } }}>
                                            <Avatar src={product.imageUrl} variant="rounded" sx={imageStyle} />
                                            {showOverlay && (
                                                <Box 
                                                    onClick={() => router.push(`/rendelesek/${order.id}`)} 
                                                    sx={{ 
                                                ...
                                                >
                                                    <Typography variant="caption" fontWeight={600}>Összes</Typography>
                                                    <Typography variant="caption" fontWeight={600}>termék</Typography>
                                                </Box>
                                            )}
                                        </Box>
                                    </Grid>
