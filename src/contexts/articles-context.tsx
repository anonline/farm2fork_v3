...
type ArticlesContextType = {
    ...
    createArticle: (
        articleData: Omit<IArticleItem, 'id' | 'categories' | 'categoryIds'>,
        categoryIds: number[]
    ) => Promise<any>;
    updateArticle: (
        id: number,
        articleData: Partial<IArticleItem>,
        categoryIds: number[]
    ) => Promise<any>;
...
export function ArticlesProvider({ children }: Readonly<{ children: ReactNode }>) {
    ...
    const fetchArticles = useCallback(async () => {
        ...
        const [articlesResponse, categoriesResponse] = await Promise.all([
            supabase
                .from('Articles')
                .select('*, ArticlesCategoriesRelations( ArticleCategories ( id, title ) )'),
            ...
            const articlesWithCategories = articlesResponse.data.map((article) => {
                ...
                return {
                    ...
                    categoryIds: relations
                        .map((rel: any) => rel.ArticleCategories?.id)
                        .filter(Boolean),
                };
            });
    ...
    const createArticle = async (
        ...
        if (!newArticle) {
            throw new Error('Hiba a cikk létrehozásakor.');
        }

        const relationsToInsert = categoryIds.map((catId) => ({
