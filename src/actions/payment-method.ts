...
export async function updatePaymentMethod(id: number, updatedData: Partial<Omit<IPaymentMethod, 'id'>>) {
    const { data, error } = await supabase
        .from('PaymentMethods')
        .update(updatedData)
        .eq('id', id)
...
export async function deletePaymentMethod(id: number) { 
    const { data: method, error: fetchError } = await supabase.from('PaymentMethods').select('protected').eq('id', id).single();
