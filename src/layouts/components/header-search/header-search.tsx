...
export default function HeaderSearch() {
    ...
    const dbProducerToSearchResult = (producer: IProducerItem): HeaderSearchProducerResultItem => ({
        id: producer.id,
        name: producer.name + (producer.companyName.length > 0 ? ' (' + producer.companyName + ')' : ''),
        image: producer.featuredImage ?? "https://placehold.co/64x64",
        description: producer.shortDescription == null || producer.shortDescription.length == 0 ? producer.producingTags : producer.shortDescription,
...
function HeaderSearchProductResultItem({ name, image, price, bio, unit }: Readonly<HeaderSearchProductResultItem>) {
    return (
        <Box sx={{ display: 'flex', border: "1px solid #dfdcd1", borderRadius: '8px', gap: '8px', alignItems: 'center', padding: "8px", width: '100%', cursor: 'pointer', backgroundColor: '#fff', '&:hover': { transform: 'scale(1.01)' } }}>
            ...
            <Box sx={{ width: '100%', alignItems: 'top', display: 'flex', height: '64px', flexDirection: 'column', gap: '4px' }}>
                <Typography
                    sx={{
                        fontSize: '15px',
                        fontWeight: '600',
                ...
                >
                    {name} {bio && <F2FIcons name="BioBadge" width={28} style={{ marginLeft: '4px' }} />}
                </Typography>
                <Typography sx={{ fontSize: '14px', fontWeight: '500', lineHeight: '20px', color: '#7e7e7e' }}>{fCurrency(price)}/{unit ?? 'db'}</Typography>
            </Box>
        </Box>
    );
}
